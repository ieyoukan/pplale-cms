...
- 1 PR = 1 カード追加/編集を基本とする（レビューしやすさのため。まとめ申請が必要な場合は要相談）
- ブランチ名: `cms/<type>-<id>` 例: `cms/yojo-y_42`
- コミットメッセージ: 変更内容 + 提出者の Discord ユーザーID（監査用）
  例: `feat: 幼女カード「〇〇」追加 (submitted by discord:123456789012345678)`
- PR 本文に最低限含める項目:
  - 変更ファイル一覧
  - 提出者の Discord ユーザー名 / ID
  - 画像変換前後のファイルサイズ（任意、大きすぎる画像の検知用）
