...
`src/data/en/*.json` は手動翻訳ではなく `scripts/import-english-cards.mjs`
(`npm run cards:import-en`) による**自動再生成**。ローカルの `assets/` 配下に置く
外部 CSV + ZIP 画像（Git 管理外、CMS の外で別途用意する）を読み込み、日本語版データの
**配列順（index）**で英語 CSV の行と対応付けて丸ごと作り直す。運用しているのはカードを
作る人とカードを実装する人の2人のみで、翻訳担当のような別チームは存在しない。
...
- CMS 側でこれを解決する術はない（外部 CSV/画像アセットは CMS の管轄外）。
  PR の説明文に「英語版データの追従が必要」であることを明記する程度に留める
- ただし PPLALE-web 側 `src/data/catalog.ts` に ID ベースのフォールバックを実装済み
  ...
  「英語CSVが用意されるまで日本語表示になる」という挙動は許容範囲という前提
