...
カードフォームの「新しいフルーツ分類を追加」「新しいお菓子タイプを追加」から、
日本語名と英語名だけで新しい分類を作成できる。最初にその分類を使うカードと同じ PR に、
PPLALE-web の型、スキーマ、日英表示名、絞り込み、2Pick の選択肢に必要な変更も含まれる。
分類の内部キーは英語名から自動生成される。
